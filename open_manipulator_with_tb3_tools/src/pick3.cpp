...
#define DIST_GRIPPER_TO_JOINT4       0.12
...
{
  open_manipulator_with_tb3_msgs::Pick msg;
...
geometry_msgs::PoseStamped calcDesiredPose()
{
  
  geometry_msgs::PoseStamped get_pose;
  geometry_msgs::Pose tmp_centroid_pose;
  
  //if (marker.id == get_marker_id)
  if ( centroid_pose_size!= 0 )
  {
    //get_pose = marker.pose;
    tmp_centroid_pose = centroid_pose[0];
    //get_pose.pose.position.x = tmp_centroid_pose.position.z - DIST_GRIPPER_TO_JOINT4;
    get_pose.pose.position.x = tmp_centroid_pose.position.z - 0.12;
    get_pose.pose.position.y = 0.0;
    get_pose.pose.position.z = tmp_centroid_pose.position.y + offset_for_object_height;
    //get_pose.pose.position.z = tmp_centroid_pose.position.y + 0.180;

    double dist = sqrt((tmp_centroid_pose.position.z * tmp_centroid_pose.position.z) +
                       (get_pose.pose.position.y * get_pose.pose.position.y));

    if (get_pose.pose.position.y > 0) yaw =        acos(tmp_centroid_pose.position.z/dist);
    else                                 yaw = (-1) * acos(tmp_centroid_pose.position.z/dist);
    ...
    ROS_INFO("x = %.3f, y = %.3f, z = %.3f, ox = %.3f, oy = %.3f, oz = %.3f, ow = %.3f, ", \
		get_pose.pose.position.x, get_pose.pose.position.y, get_pose.pose.position.z, \
		get_pose.pose.orientation.x, get_pose.pose.orientation.y, get_pose.pose.orientation.z, get_pose.pose.orientation.w );
...
bool pickMsgCallback(open_manipulator_with_tb3_msgs::Pick::Request &req,
                     open_manipulator_with_tb3_msgs::Pick::Response &res)
{
...
{  
  ...
  for(int i = 0 ; i < msg->poses.size() ; i++){
	if( !( ( msg->poses[i].position.x == 0 ) && ( msg->poses[i].position.y == 0 ) && ( msg->poses[i].position.z == 0 ) ) ) {
		...
		//ROS_INFO("%d _  %.3f, %.3f, %.3f ",i, centroid_pose[i].position.x, centroid_pose[i].position.y, centroid_pose[i].position.z);
...
void pick()
{
  ...
  switch (task)
  {
    case CHECK_AR_MARKER_POSE:
      ...
      desired_pose = calcDesiredPose();
      if (state.marker == true)
      {
        ROS_WARN("SAVE POSE OF Object");
        task = INIT_POSITION;
      }
      else
      {
        ROS_ERROR("CAN NOT FIND Object");
        task = WAITING_FOR_SIGNAL;
      }
     break;
    ...
    case CLOSE_TO_OBJECT:
      ...
      if (state.arm == STOPPED)
      {
        ...
        //object_pose.pose.position.x = object_pose.pose.position.x + 0.09;
        object_pose.pose.position.x = object_pose.pose.position.x + dist_ar_marker_to_object ;
...
int main(int argc, char **argv)
{
  ...
  pick_result_client = nh.serviceClient<open_manipulator_with_tb3_msgs::Pick>(robot_name + "/result_of_pick_up");
  ...
  ros::Subscriber centroid_pose_array_sub = nh.subscribe("/cluster_decomposer/centroid_pose_array", 10, centroidPoseArrayMsgCallback);
